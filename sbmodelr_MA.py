...
def main():
  ...
  # check if we need to add the Hill transport rate law
    ...
    # species to be transported
        ...
        vivarium_json_model = create_vivarium_file(model=args.filename, 
                                                   linkers=args.transport, 
                                                   output_json=output_json, 
                                                   transport_data=vivarium_transport_data,
                                                   number_cells=2,  #TODO: get this from pedro, arguments on how to specify number of cells
                                                   )
    ...
    # otherwise save an SBML file
      ...
      #  vivarium_json_model = create_vivarium_file(newfilename, output_json, transport=args.transport)
        
      #  output_json = os.path.splitext(newfilename)[0] +"_vivarium.json"
      #  with open(output_json, "w") as outfile:
       #     json.dump(vivarium_json_model, outfile, indent=2)
    ...
    # ## added vivarium-export
    # if args.vivarium_json:
    #     vivarium_json_model = {}
    #     vivarium_json_model['vivarium_transport'] = vivarium_transport_data
    ...
    #     create_vivarium_file(newfilename, output_json, transport=args.transport)
    #     # try:
    #     #     subprocess.run(
    #     #         ['python', vivarium_copasi_script, newfilename, output_json],
    #     #         check=True
    #     #     )
    #     #     if( not args.quiet ):
    #     #         print(f"created new Vivarium JSON model {newfilename}")
    #     # except subprocess.CalledProcessError as e:
    #     #     print("Vivarium export failed")
    #     #     print(e)
